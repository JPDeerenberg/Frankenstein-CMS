...
test("utils.js base64 encoding/decoding tests", async (t) => {
  ...
  await t.test(
    "utf8ToBase64 correctly encodes utf-8 text (emojis and accents)",
    () => {
      const input = "Café ☕ 🔥 👨‍👩‍👧‍👦";
      const expected = Buffer.from(input, "utf8").toString("base64");
      const result = sandbox.utf8ToBase64(input);
  ...
  await t.test(
    "base64ToUtf8 correctly decodes utf-8 text (emojis and accents)",
    () => {
      const expected = "Café ☕ 🔥 👨‍👩‍👧‍👦";
...
test("utils.js resolvePath tests", async (t) => {
  const code = fs.readFileSync("dev/js/utils.js", "utf8");
  const sandbox = {};
  vm.createContext(sandbox);
  vm.runInContext(code, sandbox);

  await t.test("resolves simple relative path", () => {
    assert.strictEqual(
      sandbox.resolvePath("dev/css", "style.css"),
      "dev/css/style.css",
    );
  });

  await t.test("resolves relative path with ..", () => {
    assert.strictEqual(
      sandbox.resolvePath("dev/css", "../js/app.js"),
      "dev/js/app.js",
    );
  });
  ...
  await t.test("handles absolute path", () => {
    assert.strictEqual(
      sandbox.resolvePath("dev/css", "/images/logo.png"),
      "images/logo.png",
    );
  });
});

test("prod/js/utils.js resolvePath tests", async (t) => {
  const code = fs.readFileSync("prod/js/utils.js", "utf8");
  const sandbox = {};
  vm.createContext(sandbox);
  vm.runInContext(code, sandbox);
